...
export default class Game extends Phaser.Scene {
  ...
  checkLineCollisions() {
      ...
      // Condición para pegarse a la pared (wall stick)
      ...
      // PRIMERO: Detectar colisión para pegarse
      if (canStick) {
        for (const line of lines) {
          ...
          if (collision.collided) {
            ...
            console.log("🧱 Wall stick! Line:", line);
      ...
      // SEGUNDO: Si NO está en wall run, aplicar rebotes/dash
      if (!this.player.wallRun || !this.player.wallRun.isWallRunning) {
        for (const line of lines) {
          const collision = this.lineCollisionBetween(
            { x: this.player.prevX, y: this.player.prevY },
            { x: this.player.px, y: this.player.py },
  ...
  lineCollisionBetween(p1, p2, line, radius) {
    ...
    if (distToClosest < radius) {
      ...
      if (moveLen === 0 || dotInit > 0) {
        return { collided: true, hitX: p1.x, hitY: p1.y, distance: distToClosest };
      }
    }
    ...
    return { collided: true, hitX: hitX, hitY: hitY, distance: radius };
  }
